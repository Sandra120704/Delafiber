...
class Reporte extends BaseController
{
    ...
    private function calcularKPIs($fechaInicio, $fechaFin)
    {
        ...
        // Ingresos estimados (suma de presupuestos de leads convertidos)
        $result = $db->table('leads')
            ->selectSum('presupuesto_estimado')
            ->where('fecha_registro >=', $fechaInicio)
            ->where('fecha_registro <=', $fechaFin . ' 23:59:59')
            ->where('estado', 'Convertido')
            ->get()
        ...
        $ingresos = $result->presupuesto_estimado ?? 0;

        // Ticket promedio
        $ticketPromedio = $conversiones > 0 ? $ingresos / $conversiones : 0;
    ...
    private function getRendimientoVendedores($fechaInicio, $fechaFin)
    {
        ...
        $resultado = $db->query("
            SELECT 
                ...
                COALESCE(SUM(CASE WHEN l.estado = 'Convertido' THEN l.presupuesto_estimado ELSE 0 END), 0) as ingresos,
                COALESCE(
                    SUM(CASE WHEN l.estado = 'Convertido' THEN l.presupuesto_estimado ELSE 0 END) / 
                    NULLIF(SUM(CASE WHEN l.estado = 'Convertido' THEN 1 ELSE 0 END), 0),
                    0
                ) as ticket_promedio
            FROM usuarios u
    ...
    private function getRendimientoCampanias($fechaInicio, $fechaFin)
    {
        ...
        $resultado = $db->query("
            SELECT 
                ...
                ROUND(
                    ((COALESCE(SUM(CASE WHEN l.estado = 'Convertido' THEN l.presupuesto_estimado ELSE 0 END), 0) - c.presupuesto) / 
                    NULLIF(c.presupuesto, 0)) * 100, 
                    1
                ) as roi
            FROM campanias c
